...
<?php
	...
	|| empty($_POST['Pembacaan_attaubah']) || empty($_POST['Share_medsos']) || empty($_POST['Input_muthabaah']) || empty($_POST['Jm_plg_sblm']) || empty($_POST['Aktivitas_sblm']) || empty($_POST['Pembahansan_drm'])) {
	...
	else if($jam_sekarang < $jam_boleh){
		...
		//$insert_lap = "INSERT INTO rekap_bulanan (Id_drm, nip, Tanggal) VALUES ('$Id_drm','$nip','$Tanggal')";
		//$query = mysql_query ($insert_lap);
		
		if ( $query && $jam_sekarang < $jam_boleh){
	...
	if($jam_sekarang > $jam_boleh) {
		echo "<div class='register-logo'><b>Mohon Maaf</b> waktu Habis!</div>	
		<div class='register-box-body'>
			<p>Input Data gagal periksa jam  <b>$jam_sekarang</b> </p>
			<div class='row'>
				<div class='col-xs-8'></div>
				<div class='col-xs-4'>
					<button type='button' onclick=location.href='home-pegawai.php?page=form-permohonan-drm' class='btn btn-block btn-warning'>Back</button>
